...
// (S-015 OobScreen) and the new pairing screen (S-016 PairingScreen).
//
...
// route renders the S-015 OobScreen plus a new "Pair" button that routes
// to the pair route.
//
...
//
// Production wiring for S-016 (BluetoothBondRemover, OobCalculator,
// BondPersister, PairBackend) is constructed at the Activity level so
// the ViewModel never reaches into the global Application graph. The
// PairBackend is currently a stub (S-018 ships the real Bluetooth-stack
// impl); the OobCalculator is the real UniFFI-backed one; the
// BluetoothBondRemover is the real reflection-backed one; the
// BondPersister is a stub that will be replaced by a UniFFI-exposed
// bond keystore in a future step.
...
private fun SyauthApp(applicationContext: Context) {
    ...
    NavHost(navController = navController, startDestination = Routes.HOME) {
        composable(Routes.HOME) {
            HomeRoute(
                onPairTapped = { navController.navigate(Routes.PAIR) },
            )
        }
